...
fig, axis=plt.subplots(7, 5, figsize=(15, 15))
random_imgs=random.sample(range(output_01.shape[0]), 5)
output=[
    x_test, output_01, output_02, output_04, 
    output_08, output_16, output_32
]

for row_num, row in enumerate(axis):
    for col_num, ax in enumerate(row):
        ax.imshow(output[row_num][random_imgs[col_num]].reshape(28, 28),
            cmap='gray')
        ax.grid(False)
        ax.set_xticks([])
        ax.set_yticks([])
...
# for i in range(6):
#     a=n**i
#     model_a=autoencoder(hidden_layer_size=a)
#     print('node' + str(a) + '개 시작')
#     model_a.compile(
#         optimizer='adam',
#         loss='binary_crossentropy'
#     )
...
#     output_a=model_a.predict(x_test)


#     fig, axis=plt.subplots(7, 5, figsize=(15, 15))
#     random_imgs=random.sample(range(output_a[1].shape[0]), 5)
#     output=[
#         x_test, output_a
#     ]
