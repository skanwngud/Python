...
from keras.layers import Dense, Input, Conv2D, Flatten, UpSampling2D, Conv2DTranspose
...
x_train_noised_2=x_train_2+np.random.normal(0, 0.1, size=x_train_2.shape) # random.normal = 정규분포에 맞게 0부터 0.1 사이의 수치를 점으로 찍음
x_test_noised_2=x_test_2+np.random.normal(0, 0.1, size=x_test_2.shape) # x 의 값이 0~1 이기 때문에 0~1.1 의 범위를 갖게 되며 전체적으로 0.1 만큼 밝아짐
x_train_noised_2=np.clip(x_train_noised_2, a_min=0, a_max=1) # 최소 최대값을 0~1 로 고정 시켜줌
x_test_noised_2=np.clip(x_test_noised_2, a_min=0, a_max=1)
...
for i, ax in enumerate([ax1, ax2, ax3, ax4, ax5]):
    ax.imshow(x_test_2[random_images[i]].reshape(28, 28), cmap='gray')
    if i==0:
