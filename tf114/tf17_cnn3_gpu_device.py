...
b1 = tf.compat.v1.Variable(
    tf.compat.v1.random_normal([32]),
    name = 'bias1'
)
...
b3 = tf.Variable(
    tf.compat.v1.random_normal([128]),
    name = 'bias3'
)
...
L5 = tf.nn.dropout(L5, 0.7)

# w6
w6 = tf.compat.v1.get_variable(
    'w6', shape = [64, 32],
    # initializer=tf.contrib.layers.xavier_initializer()
)
...
L6 = tf.nn.dropout(L6, 0.7)

# w7
w7 = tf.compat.v1.get_variable(
    'w7', shape=[32, 10],
    # initializer=tf.contrib.layers.xavier_initializer()
)

b7 = tf.Variable(
    tf.compat.v1.random_normal([10]),
    name = 'bias7'
)
